...
### 存储一个多面体
想要如何存储一个多面体，我们记得在数学中可以这样来标记。我们在学校学过一些基本的几何图形，为了表示一个正方形，我们把它叫做ABCD ,使用A，B，C，D来表示四个顶点以构成一个正方形。
...
Cube类的构造函数通过生成立方体的顶点来确定一个立方体，这些顶点的位置是通过指定中心点位置与立方体大小计算出来的。这个模式就快要变得清晰了，我们在下图可以看到这8个顶点的位置。
...
### 需要三角形吗?
如果之前使用过3D技术(比如Blender，webGL等),那么你可能听说过我们应该使用三角形。在这篇文章中，我没有选择使用三角形(注: 确定一个面只需要三个点就可以了)。
...
### 让多面体动起来
还有一个存储顶点引用而不是存顶点副本的优势是，假如我们要修改这个多面体的时候，还能将本来的三个操作简化为了一个操作。(注: 比如保存了三个相同的顶点，如果对这个顶点进行移动，那么需要对这三个点都进行操作才正确，而如果使用引用的方式的话，只需要移动一个点就可以了。)

为什么呢? 我们来回顾一下数学课上学的东西.当想要平移一个正方形的时候，实际上你不需要真正的去平移这个正方形，你只需要平移正方形的四个顶点就可以了。

在这里，我们也是相同的做法：我们不会去触碰正方体的那些面，我们只需要把想要的操作应用到每个顶点上就可以了。那些面的坐标都是使用引用的方式创建的，只要坐标改变了，那么对应的面也会改变。下面这个实例将展示我们如何平移我们创建的立方体：
```javascript
for (var i = 0; i < 8; ++i) {
...
# 渲染图像
现在我们知道了如何存储一个3D对象并且知道了如何对它们进行一些动作。是时候该看见它们了!但是，为了更能明白我们需要做什么，还需要一小点背景知识。

### 投影
目前，我们存储了3D坐标。但是，一个屏幕只能展现二维坐标，因此需要有一种方法将3D坐标转化为2D坐标: 这在数学上我们称之为投影(projection)。3D到2D的投影是一个抽象操作，是通过一个叫做虚拟摄像机的东西来完成的。这个相机将3D对象坐标转换为2D的，然后把它们发送到屏幕上面渲染出来。我们假设这个相机在我们3D空间中的原点上面(坐标是(0,0,0)).
...
现在我们知道我们要做什么了：使用(x, y, z)表示坐标，为了在屏幕上显示他们，我们需要将它转换成(x, z)的形式。因为屏幕是一个平面，为来能够显示出3D效果。

这里不仅仅只存在一个投影，更糟糕的是存在无限多不同的投影。在此文中我们可以看到两种不同类型的投影，这两种也是在实践中是最常使用的。

### 如何绘制场景
在展现图形之前，我们先写一个函数来绘制。这个函数接收一个对象数组(数组中每个对象是一个Cube实例)，第二个参数是canvas的contex对象，其它参数用来让图形被绘制在一个正确的位置上。
...
具体的project()函数的实现你将会在下一节中看到, 它取决于你选择的投影类型。但是不管是什么类型，我们编写的render()函数是不会改变的。

坐标一旦在平面上了，我们就可以把它们绘制在canvas上面，这就是我们想做的。这里有一个小技巧是我们不会真正的绘制由project()函数返回回来的坐标点。

实际上project()函数返回的是一个虚拟2D平面上的点，它的原点和我们定义的3D空间中的原点是一样的。但是我们想要原点在canvas画布的正中心位置上，那就是为什么要平移坐标了。顶点(0, 0)不在canvas画布的中心,但是(0+dx, 0+dy)在。因此只需要将dx=canvas.width/2,dy=canvas.height/2就可以了。
...
### 正视图
让我们从正视图开始，这也是最简单的一种视图，他能很好的帮助我们理解。
...
要理解为什么，我们用一个图示来表示正投影，用正交的方式将我们的点投影到一个平面上。
